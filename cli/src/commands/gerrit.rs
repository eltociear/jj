...
use crate::cli_util::{
    ...
    RemoteBranchNamePattern, RevisionArg,
};
...
pub fn cmd_gerrit(
    ...
    match subcommand {
        GerritCommand::Mail(review) => cmd_mail(ui, command, review),
    }
}
...
pub fn cmd_mail(ui: &mut Ui, command: &CommandHelper, mail: &MailArgs) -> Result<(), CommandError> {
    let mut workspace_command = command.workspace_helper(ui)?;

    let to_mail: IndexSet<Commit> =
        resolve_multiple_nonempty_revsets(&mail.revision, &workspace_command, ui)?;
    if to_mail
        .iter()
        .any(|commit| commit.id() == workspace_command.repo().store().root_commit_id())
    {
        return Err(user_error("Cannot mail the root commit"));
    }

    workspace_command.check_rewritable(to_mail.iter())?;
    ...
    let remote_pattern =
        RemoteBranchNamePattern::from_str(&mail.for_).map_err(|err| user_error(err.to_string()))?;
            ...
            if let Some((_, values)) = change_id {
                ...
                if values.len() > 1 {
                    writeln!(
                        ...
                        short_commit_hash(original_commit.clone().id()),
                    )?;
                    continue;
                }

                // check the change-id format is correct in any case
                if values[0].len() != 41 || !values[0].starts_with('I') {
                    writeln!(
                        ui.warning(),
    ...
    tx.finish(
        ui,
        format!(
            "describing {} commit(s) for mail-in to gerrit",
            old_to_new.len()
        ),
    )?;
    ...
    for head in &new_heads {
        ...
        // XXX (aseipp): for yuya -- is this roughly correct? i don't think we
        // want to record or update jj's state on the remote refs when we push,
        // since it's special and synthetic ref in the first place.
        //
